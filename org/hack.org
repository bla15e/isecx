...
** guix-infect.sh
#+begin_src shell :tangle ../hack/guix-infect/debian-infect.sh
  #!/bin/bash
  ...
  (cons*
        (channel
          (name 'isecx)
          (url "https://gitlab.com/bla15e/gc")
          (branch "main"))
        %default-channels)
  EOL
  }
...
#+begin_src shell :tangle ../hack/build-channel.sh :noweb yes :comments noweb :tangle-mode (identity #o755)
  <<hackscript-prefix>>
  main() {
    tempfile=$(mktemp)
    cat << EOF > "$tempfile"
